<%@ Control Language="C#" ClassName="BbPatientDlqiFields" %>
<script runat="server">
        ...
        <fieldset class="fFupId" style="display: none">
            ...
            <ol>
                <li>
                    ...
                    <asp:TextBox runat="server" ID="dataFupId" Text='<%# Bind("FupId") %>' Enabled="false" CssClass="disabled" />
                </li>
            </ol>
        </fieldset>
        ...
        <fieldset>
            ...
            <ol>
                ...
                <li>
                    ...
                    <asp:TextBox runat="server" ID="dataDatecomp" Text='<%# Bind("Datecomp", "{0:d}") %>' MaxLength="10"></asp:TextBox><asp:ImageButton ID="cal_dataDatecomp" runat="server" SkinID="CalendarImageButton" OnClientClick="javascript:showCalendarControl(this.previousSibling);return false;" />
                    <asp:CompareValidator runat="server" ID="valdataDatecomp" ControlToValidate="dataDatecomp" Operator="DataTypeCheck" Type="Date" Display="Dynamic" ErrorMessage="Please enter a valid date, in dd/mm/yyyy format" />
                    <asp:RequiredFieldValidator runat="server" ID="RFDate" ControlToValidate="dataDatecomp" ErrorMessage="* Required" />
                    <asp:RangeValidator runat="server" ID="pqDateRangeVal" ControlToValidate="dataDatecomp" ErrorMessage="Date must be after the patient's date of birth and not in the future" Display="Dynamic"
                        Type="Date" MinimumValue='<%# Session["rvMinPatientDOB"].ToString() %>' MaximumValue='<%# DateTime.Now.ToString("dd/MM/yyyy") %>'/>
                </li>
            </ol>
        </fieldset>
